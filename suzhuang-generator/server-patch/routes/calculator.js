...
// 诉讼费计算（中国法院标准）
function calculateLitigationFee(amount, caseType, nonPropertyType, halfFee, waiver) {
  let fee = 0;

  if (caseType === 'nonProperty') {
    switch (nonPropertyType) {
      case 'divorce': fee = 300; break;
      case 'labor': fee = 10; break;
      case 'personality': fee = 500; break;
      case 'intellectual': fee = 500; break;
      default: fee = 80;
    }
  } else {
    // 财产案件，按标的额分段累计交纳
...
router.post('/interest', (req, res) => {
  ...
  if (rateType === 'month') yearRate = yearRate * 12;
  else if (rateType === 'day') yearRate = yearRate * 365;
  ...
  return res.json({
    code: 200,
    message: 'success',
    data: { interest: roundedInterest, days, details }
  });
});
...
router.post('/penalty', (req, res) => {
  ...
  let rateDesc = '';
  switch (rateMode) {
    case '1.3': penaltyRate = Number(contractRate) * 1.3; rateDesc = '上浮30%'; break;
    case '1.5': penaltyRate = Number(contractRate) * 1.5; rateDesc = '上浮50%'; break;
    case '2': penaltyRate = Number(contractRate) * 2; rateDesc = '上浮100%'; break;
    case 'fixed': penaltyRate = Number(fixedRate || contractRate); rateDesc = `固定利率${penaltyRate}%`; break;
    default: penaltyRate = Number(contractRate) * 1.5; rateDesc = '上浮50%（默认）';
  }
  ...
  return res.json({
    code: 200,
    message: 'success',
    data: { penaltyInterest: roundedPenaltyInterest, days, details }
  });
});
...
router.post('/compound', (req, res) => {
  ...
  if (method === 'simple') {
    ...
    else if (period === 'quarter') periodDays = 90;
  ...
  return res.json({
    code: 200,
    message: 'success',
    data: { compoundInterest: roundedCompoundInterest, details }
  });
});
...
router.post('/breach', (req, res) => {
  ...
  return res.json({
    code: 200,
    message: 'success',
    data: { amount: roundedAmount, details }
  });
});
